...
세 모드는 오행 조건값을 다르게 사용하지 않습니다. 입력 시각·시간대·도시·명식으로 만든 로컬 해시가 발현 여부만 정하므로, 같은 입력이 관문을 통과했다면 어느 모드에서도 같은 영근 종류가 나옵니다.

오행 점수는 주근과 부근의 강약 순위를 정하고, 별도의 결정적 순도 배분값이 몇 개의 오행 통로가 안정되는지 정합니다. 품질은 `천영근 > 변이영근 > 이영근 > 삼영근 > 사영근 > 오영근` 순이며 보유자 기준 목표 비율은 다음과 같습니다.
...
달력 테스트에는 라이브러리 관리자가 README에 공개한 `1986-05-29` 예시(병인년·계사월·계유일), 2024년 입춘/절입 전후, 자시, 야자시, 시간대, 윤년, 2023년 윤2월, 진태양시를 포함합니다. 영근 테스트는 인공 명식 데이터로 0~5개 분류, 혼원, 빙·뇌·풍·용암 변이, 경계 후보와 제3 영근 방해를 검증합니다. 추가로 균형·엄격 관문의 15%·1% 분포, 5,000개 결정 표본의 품질 목표 범위, 엄격 모드 통과자의 영근 종류가 유연 모드와 동일한지 검사합니다.

...
30,000개 결정 표본의 상위 분위수를 기준으로 균형 관문은 56.5점(약 15%), 엄격 관문은 68.8점(약 1%)으로 보정했습니다. 별도의 20,000개 표본에서는 14.69%와 1.07%가 발현했습니다.

오행 점수는 월령의 왕·상·휴·수·사, 투출·통근·지장간, 합충형파해, 상생 유통과 통관을 합산해 주근과 부근의 강약 순위를 정합니다. 별도의 결정적 순도 배분값은 몇 개의 오행 통로가 안정되는지 정합니다. 품질은 `천영근 > 변이영근 > 이영근 > 삼영근 > 사영근 > 오영근` 순이며 보유자 기준 목표 비율은 다음과 같습니다.
...
달력 테스트에는 라이브러리 관리자가 README에 공개한 `1986-05-29` 예시(병인년·계사월·계유일), 2024년 입춘/절입 전후, 자시, 야자시, 시간대, 윤년, 2023년 윤2월, 진태양시를 포함합니다. 영근 테스트는 인공 명식 데이터로 0~5개 분류, 혼원, 빙·뇌·풍·용암 변이, 순천·역천도맥을 검증합니다. 추가로 균형·엄격 관문의 15%·1% 표본 범위, 5,000개 결정 표본의 품질 목표 범위, 엄격 모드 통과자의 영근 종류가 유연 모드와 동일한지 검사합니다.

...
기본값인 **유연 판정**은 영근 보유를 전제로 합니다. 1.5점 이상이면서 다음 통로 중 하나가 확인된 오행을 작동 영근 후보로 봅니다.
...
**엄격 판정**은 위 분석 전에 인구 약 1%의 결정적 발현 관문을 적용합니다. 입력 시각·시간대·도시·명식으로 만든 로컬 해시를 사용하므로 서버 전송 없이 같은 입력은 항상 같은 결과가 나옵니다. 이 1% 관문은 전통 명리학이 아니라 선협 인구 설정입니다.

영근 품질은 `천영근 > 변이영근 > 이영근 > 삼영근 > 사영근 > 오영근` 순입니다. 반대로 보유자 내 빈도는 오영근이 가장 높고 천영근이 가장 낮도록 표본 분포 테스트로 보호합니다. 한 기맥이 압도적인 경우에만 천영근으로 정제하고, 변이는 두 유효 원재료와 융합 조건이 모두 맞아야 합니다. 신살은 영근 개수를 바꾸지 않고 부가 성향에만 사용됩니다.
...
달력 테스트에는 라이브러리 관리자가 README에 공개한 `1986-05-29` 예시(병인년·계사월·계유일), 2024년 입춘/절입 전후, 자시, 야자시, 시간대, 윤년, 2023년 윤2월, 진태양시를 포함합니다. 영근 테스트는 인공 명식 데이터로 0~5개 분류, 혼원, 빙·뇌·풍·용암 변이, 경계 후보와 제3 영근 방해를 검증합니다. 추가로 엄격 관문의 약 1% 분포와 5,000개 결정 표본에서 `오 > 사 > 삼 > 이 > 변이 > 천` 빈도 순서가 유지되는지 검사합니다.

...
import { CONTROLS, ELEMENTS, GENERATES, generatorOf } from "@/lib/bazi/elementMeta";
...
export function calculateDaoAffinity(
  ...
  const strongElements = ELEMENTS.filter((element) => evidence[element].score >= SPIRITUAL_ROOT_RULES.thresholds.independentStrength);
  const rootedVisible = ELEMENTS.filter((element) => evidence[element].roots.length > 0 && evidence[element].visibleStems.length > 0).length;
  ...
  const generatingLinks = ELEMENTS.filter((element) =>
    evidence[element].score >= SPIRITUAL_ROOT_RULES.thresholds.independentStrength &&
    evidence[GENERATES[element]].score >= SPIRITUAL_ROOT_RULES.thresholds.potential).length;
  const mediationCount = ELEMENTS.filter((element) => {
    const source = generatorOf(element);
    const destination = GENERATES[element];
    return CONTROLS[source] === destination &&
      evidence[source].score >= SPIRITUAL_ROOT_RULES.thresholds.independentStrength &&
      evidence[destination].score >= SPIRITUAL_ROOT_RULES.thresholds.independentStrength &&
      evidence[element].score >= SPIRITUAL_ROOT_RULES.thresholds.potential;
  }).length;
  ...
  const rescued = day.roots.length > 0 && (day.supportScore > 0 || generator.score >= SPIRITUAL_ROOT_RULES.thresholds.independentStrength);
  ...
  add("natural", "득지", Math.min(day.roots.length, 3) * natural.dayRoot, `일간이 지지 ${day.roots.length}곳에 통근했습니다.`);
  if (day.supportScore > 0 || generator.score >= 7) add("natural", "생조", natural.daySupport, "일간을 생하는 인성 기운이 실제 세력을 갖췄습니다.");
  ...
  add("natural", "천간합", Math.min(relations.stemCombinations.length, 2) * natural.stemCombination, `천간합 ${relations.stemCombinations.length}건이 결속을 만듭니다.`);
  ...
  if (day.roots.length === 0) add("natural", "무근", natural.rootlessPenalty, "일간이 지지에 뿌리를 두지 못했습니다.");
  ...
  add("defiant", "역근", Math.min(day.roots.length, 3) * defiant.dayRoot, `불리한 기세 속에서도 일간이 ${day.roots.length}곳에 뿌리를 내립니다.`);
  if (day.supportScore > 0 || generator.score >= 7) add("defiant", "구원", defiant.daySupport, "생조 기운이 역세를 돌파할 구원 통로가 됩니다.");
  ...
  if (day.controlPenalty < 0 && day.roots.length > 0) add("defiant", "극중유근", defiant.controlledButRooted, "강한 극을 받으면서도 일간의 뿌리가 남아 있습니다.");
  if (day.roots.length === 0) add("defiant", "무근", defiant.rootlessPenalty, "역천의 기반이 될 일간의 뿌리도 없습니다.");
  ...
  const path = naturalScore >= defiantScore ? "natural" : "defiant";
  const tieBreaker = stableSpiritualRootRoll(`${seed}::dao`);
  const score = rounded(Math.max(naturalScore, defiantScore) + tieBreaker / 10_000);

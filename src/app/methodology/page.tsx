...
export default function MethodologyPage() {
  return <InfoPageShell eyebrow="Methodology" title="계산법과 한계" intro="달력 계산의 기준, 시간 보정 방식, 라이브러리 선택과 불확실성을 투명하게 공개합니다." disclaimer>
    ...
    <InfoSection title="영근 발현 모드와 희귀도"><p>입력 시각·시간대·도시·명식으로 만든 FNV-1a 결정값을 사용해 유연 100%, 균형 약 15%, 엄격 약 1%의 발현 관문을 적용합니다. 모드는 오행 점수 문턱을 바꾸지 않으며 발현 여부만 결정합니다. 통과 후에는 별도의 순도 배분값과 실제 오행 강약을 결합해 보유자 기준 천 1~2%, 변이 4~6%, 이 10~14%, 삼 18~22%, 사 26~30%, 오 30~36%를 목표로 합니다. 변이는 구조 조건을 추가로 충족해야 하므로 실패분은 이영근으로 남습니다. 모든 값은 외부 전송이나 저장 없이 결정되며 전통 명리가 아닌 선협 창작 규칙입니다.</p></InfoSection>

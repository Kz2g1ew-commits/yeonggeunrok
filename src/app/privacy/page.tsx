...
export default function PrivacyPage() {
  return <InfoPageShell eyebrow="Privacy by design" title="출생 정보는 저장하지 않습니다" intro="생년월일시는 민감한 정보입니다. 영근록의 초기 버전은 계산에 필요한 값을 현재 브라우저 메모리 안에서만 처리합니다.">
    ...
    <InfoSection title="안전한 결과 이미지" marker="三"><p>‘결과 이미지 PNG 다운로드’는 최종 영근 판정, 목·화·토·금·수의 기맥 활성, 선협 재능 종합표와 수련 설정만 브라우저 안에서 PNG 파일로 만듭니다. 생년월일시, 출생지, 성별, 사주팔자 원문은 포함하지 않으며 이미지나 입력값을 서버 또는 클립보드로 전송하지 않습니다.</p></InfoSection>

...
export default function PrivacyPage() {
  return <InfoPageShell eyebrow="Privacy by design" title="출생 정보는 저장하지 않습니다" intro="생년월일시는 민감한 정보입니다. 영근록의 초기 버전은 계산에 필요한 값을 현재 브라우저 메모리 안에서만 처리합니다.">
    ...
    <InfoSection title="안전한 결과 공유" marker="三"><p>‘민감정보 없이 결과 이미지 복사’는 최종 영근 이름과 목·화·토·금·수의 원국 구성비만 브라우저 안에서 PNG 카드로 만듭니다. 생년월일시, 출생지, 성별, 사주 원문은 포함하지 않으며 이미지를 서버에 업로드하지 않습니다. 이미지 클립보드를 지원하지 않는 환경에서는 기기 공유 창을 열거나 PNG 파일로 저장합니다.</p></InfoSection>

...
const mutations = [
  ...
  ["정 晶", "토 + 금", "토생금, 양쪽 통근, 결정의 안정적 응축과 균형", "토다금매·강한 화·토근 파쇄"],
  ["암 暗", "수 + 토", "한습, 침잠, 귀문 보조", "강한 화"],
  ["운 雲", "수 + 화", "중화·윤습의 수화기제형 또는 한랭·한습 속 깊은 양근의 응결형", "수화의 직접 격충·무근·양근의 심한 편중"],
...
export default function MutationsPage() {
  return <InfoPageShell eyebrow="Mutation archive" title="변이영근 도감" intro="두 오행이 있다는 이유만으로 변이가 되지는 않습니다. 유효한 원재료, 관계의 강도, 계절의 지지, 점수 균형과 방해 요소를 함께 확인합니다.">
    ...
    <InfoSection title="네 단계의 판정"><p><strong className="text-[#e0ce9f]">확정</strong>은 원재료 두 영근이 완전히 융합해 변이 단영근으로 작동하는 상태입니다. 그 아래는 유력, 가능성, 불성립으로 나뉩니다. 제3 유효 영근이 독립적으로 강하면 확정을 막으며, 기본 이영근이나 삼영근에 변이 후보를 덧붙여 표시합니다.</p><p className="mt-3 text-xs text-[#778c94]">정(晶)은 토석 속 금기가 옥정·수정·금강정으로 응축되는 계열입니다. 운(雲)은 중화된 수화의 기화·응결뿐 아니라 한습 속에서 깊은 화근과 수근이 맞물린 응결도 운무·환광으로 다룹니다.</p></InfoSection>
  </InfoPageShell>;
}

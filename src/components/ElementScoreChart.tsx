...
export function ElementScoreChart({ evidence }: { evidence: Record<Element, ElementEvidence> }) {
  ...
  return (
    <section className="surface p-5 sm:p-7">
      ...
      <p className="muted mt-2 text-sm">4점선 이상이면서 발현 조건을 만족해야 유효 영근입니다. 2~3.9점은 잠재 영근으로 남습니다.</p>
      <div className="mt-7 grid gap-4">
        {ELEMENTS.map((element) => { const item = evidence[element]; return <div key={element} className="surface-soft p-4">
          ...
          <div className="relative mt-3 h-2 overflow-hidden rounded-full bg-black/25"><div className="absolute inset-y-0 z-10 w-px bg-[#e6d087]/60" style={{ left: `${(4 / max) * 100}%` }} /><div className="h-full rounded-full transition-all" style={{ width: `${Math.max(0, item.score / max * 100)}%`, background: ELEMENT_META[element].color }} /></div>

...
export function ElementScoreChart({ evidence }: { evidence: Record<Element, ElementEvidence> }) {
  ...
  return (
    <section className="surface p-5 sm:p-7">
      ...
      <p className="muted mt-2 text-sm">본기·중기·여기를 강·중·약근으로 나누고, 성립된 기맥만 생조·유통·통관에 참여시킵니다. 구조 유효성 판정을 먼저 통과한 오행만 순도 배분의 대상이 됩니다.</p>
      <div className="mt-7 grid gap-4">
        {ELEMENTS.map((element) => { const item = evidence[element]; return <div key={element} className="surface-soft p-4">
          ...
          <div className="relative mt-3 h-2 overflow-hidden rounded-full bg-black/25"><div className="h-full rounded-full transition-all" style={{ width: `${Math.max(0, item.score / max * 100)}%`, background: ELEMENT_META[element].color }} /></div>
          ...
          <p className="mt-3 text-[10px] leading-5 text-[#71858e]">구조 관문: {item.structuralEligible ? item.eligibilityReasons.join(" · ") : "점수 또는 투출·통근 연결 부족"}</p>

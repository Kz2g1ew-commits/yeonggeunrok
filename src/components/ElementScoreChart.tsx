    ...
    <section className="surface p-5 sm:p-7">
      ...
      <div className="mt-7 grid gap-4">
        {ELEMENTS.map((element) => { const item = evidence[element]; return <div key={element} className="surface-soft p-4">
          ...
          {item.rootDetails.length > 0 && <p className="mt-1 text-[10px] leading-5 text-[#71858e]">뿌리: {item.rootDetails.map((root) => `${root.branch}${root.role === "main" ? " 본기" : root.role === "middle" ? " 중기" : " 여기"}${root.clashState === "activated" ? "(충발)" : root.clashState === "shaken" ? "(충동)" : root.clashState === "damaged" ? "(충손)" : root.clashState === "uprooted" ? "(발근)" : ""}`).join(" · ")}</p>}

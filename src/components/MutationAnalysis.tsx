    ...
    <section className="surface p-5 sm:p-7">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        {items.map((candidate) => <article key={candidate.id} className="surface-soft overflow-hidden p-4">
          <div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h3 className="display-serif text-xl text-[#eee2c5]">{candidate.name} 변이영근</h3><span className="rounded-full bg-[#d8b66a]/10 px-2 py-1 text-[10px] font-extrabold text-[#dfc270]">{statusLabel[candidate.status]}</span></div><div className="mt-2 flex gap-1.5">{candidate.sourceElements.map((element) => <ElementBadge key={element} element={element} compact />)}</div></div><strong className="display-serif text-2xl text-[#efd48d]">{candidate.confidence}<small className="text-xs">%</small></strong></div>

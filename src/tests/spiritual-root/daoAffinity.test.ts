...
describe("calculateDaoAffinity", () => {
  it("recognizes a rooted, supported and quietly flowing chart as a natural Dao path", () => {
    ...
    const result = calculateDaoAffinity(testPillars, evidence, context.relations);
  ...
  it("recognizes hostile season, conflict and rescue as a defiant Dao path", () => {
    ...
    const result = calculateDaoAffinity(pillars, evidence, relations);
  ...
  it("is deterministic for the same chart", () => {
    ...
    expect(calculateDaoAffinity(testPillars, evidence, relations))
      .toEqual(calculateDaoAffinity(testPillars, evidence, relations));
  });
});
